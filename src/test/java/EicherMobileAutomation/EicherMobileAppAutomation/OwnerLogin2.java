...
public class OwnerLogin2  extends base{

	
	@Test
	
	public void loginToApp() throws SQLException, IOException 
	{
	...
	ResultSet rs = s.executeQuery("select * from CustomerDetails where ID=1");
	 ...
	 while(rs.next())
	 {
		 ...
		 AndroidDriver<AndroidElement>driver = Capabilities("CEP");
			...
			u.scrollToText("LOGIN");
			//Login
			//driver.findElementByAndroidUIAutomator("new UiScrollable(new UiSelector()).scrollIntoView(text(\"LOGIN\"))");
			
			LoginPage loginPage = new LoginPage(driver);
			loginPage.clickLoginButton().click();
			loginPage.mobileNumberTextbox.sendKeys(rs.getString("LOGIN_ID"));
			//loginPage.passwordTextbox.sendKeys(rs.getString("PASSWORD"));
